...
classes = [d for d in os.listdir(data_dir) if os.path.isdir(os.path.join(data_dir, d)) and d != class_to_remove]
...
for c in classes:
    ...
    if len(images) >= target_images_per_class:
        ...
        for img_file in selected_images:
            src = os.path.join(data_dir, c, img_file)
            dst = os.path.join(balanced_dir, c, img_file)
        ...
        n_to_generate = target_images_per_class - len(images)
        for i in range(n_to_generate):
            img_file = random.choice(images)
            img_path = os.path.join(data_dir, c, img_file)
...
for c in classes:
    ...
    train_imgs = images[:target_images_per_class - val_images - test_images]
    val_imgs = images[target_images_per_class - val_images - test_images:target_images_per_class - test_images]
    test_imgs = images[target_images_per_class - test_images:]
...
# Descongelar últimas 5 camadas convolucionais para fine-tuning
num_unfreeze = 5
for block in model.features[-num_unfreeze:]:
...
for epoch in range(num_epochs):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save(best_model_wts, r"D:\Universidade\Lic_IACD\2023-2024\2Semestre\1Semestre\Programacao\Linguagens\Python\projeto\Processamento-de-Dados-Audiovisuais\Metricas\best_model.pth")
    else:
        counter += 1
        print(f"Sem melhoria: {counter}/{patience}")
        if counter >= patience:
            print("Paragem antecipada.")
            break
